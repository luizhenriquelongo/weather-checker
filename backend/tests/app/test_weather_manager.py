...
def test_get_cached_cities_weather_with_empty_cache(mocker):
    ...
    weather_data, status_code = wm.get_cached_cities_weather(DefaultConfig.MAX_CITIES_TO_RETRIEVE)
...
def test_get_city_weather_from_cache(mocker, cached_cities):
    ...
    assert weather_data.get('city') == city_name
    assert status_code == 200
...
def test_clean_data(mocker):
    city_name = "Pindamonhangaba"
    data = {'weather': [{'main': 'Sunny'}], 'main': {'temp': 20}, 'name': city_name}
